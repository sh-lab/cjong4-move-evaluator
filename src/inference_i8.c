...
bool cj4me_quantize_input_i8(const cj4me_model_i8 *model,
  ...
  if (!model || !input || !output || !isfinite(model->input_scale) ||
      model->input_scale <= 0.0f) {
    return false;
  }
  for (size_t i = 0; i < CJ4ME_FEATURE_COUNT; ++i) {
    ...
    if (!isfinite(input[i]))
      return false;
    scaled = input[i] / model->input_scale;
    if (!isfinite(scaled)) {
...
bool cj4me_infer_i8(const cj4me_model_i8 *model,
  ...
  if (!model || !input || !scratch || !output ||
      ...
      !dense_i8_relu(input, CJ4ME_FEATURE_COUNT, &model->weights1[0][0],
                     model->biases1, model->requant_multipliers1,
                     model->requant_shifts1, CJ4ME_MODEL_HIDDEN1_COUNT,
                     scratch->hidden1) ||
      !dense_i8_relu(scratch->hidden1, CJ4ME_MODEL_HIDDEN1_COUNT,
                     &model->weights2[0][0], model->biases2,
                     model->requant_multipliers2, model->requant_shifts2,
      ...
      !dense_i8_relu(scratch->hidden2, CJ4ME_MODEL_HIDDEN2_COUNT,
                     ...
                     model->requant_multipliers3, model->requant_shifts3,
  ...
  for (size_t in = 0; in < CJ4ME_MODEL_HIDDEN3_COUNT; ++in) {
    sum += (int32_t)scratch->hidden3[in] * (int32_t)model->weights4[0][in];
  }
